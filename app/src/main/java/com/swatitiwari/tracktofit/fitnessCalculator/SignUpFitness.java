...
public class SignUpFitness extends AppCompatActivity {
	...
	PhoneAuthProvider.OnVerificationStateChangedCallbacks mCallbacks = new PhoneAuthProvider.OnVerificationStateChangedCallbacks() {
		...
		public void onVerificationFailed(FirebaseException e) {
			Toast.makeText(SignUpFitness.this, "Something fishy happened!", Toast.LENGTH_SHORT).show();
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.activity_signup_fitness);
		...
		signup.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				...
				if (a.isEmpty()) {
					ageLay.setError("Enter valid age!");
					age.requestFocus();
					return;
				}
				if (h.isEmpty()) {
					heightLay.setError("Enter valid height!");
					height.requestFocus();
					return;
				}
				if (w.isEmpty()) {
					weightLay.setError("Enter valid weight!");
					weight.requestFocus();
					return;
				}
				if (s.length() != 1) {
					sexLay.setError("Enter valid sex character!");
					sex.requestFocus();
					return;
				}
				if (o.isEmpty()) {
					otpLay.setError("Enter valid OTP!");
					otp.requestFocus();
					return;
				} else {
					...
					databaseReference.addListenerForSingleValueEvent(new ValueEventListener() {
						...
						public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
							if (dataSnapshot.child(users.getMob()).exists()) {
								...
								databaseReference.child(users.getMob()).setValue(users);
								Toast.makeText(getApplicationContext(), "SignUpFitness successful!!!", Toast.LENGTH_SHORT).show();
								progressDialog.cancel();
								mAuth.signOut();    // added this
								finish();
							}
